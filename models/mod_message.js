...
function createMessage(data) {
    ...
    return db.execute(sql1).then(([rows, meta]) => {
        let sql2 = "insert into user_message (thread_id, user_sender_id, user_receiver_id, message_content) values  (" + rows.insertId + ", " + data.sender_id + "," + data.receiver_id + ", '" +data.message+"')";
        return db.execute(sql2).then(([rows, meta]) => {
...
function writeMessage(data) {
    let sql2 = "insert into user_message (thread_id, user_sender_id, user_receiver_id, message_content) values  ("+data.thread_id+ "," +data.sender_id+ ", " +data.receiver_id+ ", '" +data.message+"')";
    return db.execute(sql2).then(([rows,meta]) => {
...
function getThreads(user_id){
    ...
    let sql = "select distinct tbl.*, u.user_name from "
    ...
    sql += ")) as tbl INNER JOIN user u on u.user_id = tbl.user_id ORDER BY date_updated DESC";
